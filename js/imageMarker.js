...
var signatureMarker = function(objsignature, heading, saveMethod, deleteMethod, photoID, readOnly)
{
    this.objsignature = objsignature; 		// Assign the signature object to the local class for future reference
    this.heading = heading;			// The heading to assign to the h1 tag.
    this.saveMethod = saveMethod;	// The callback method to invoke when the signature is saved.
    this.deleteMethod = deleteMethod;// The callback method to invoke when the signature is deleted.
    this.canvas = null; 			// A reference to the canvas object
    this.context = null;			// The canvas context
    this.touchEvent = null;			// Either 'click' or 'touchstart' depending on device type
    this.closeMethod = null;		// Can be set to a callback method and will be invoked when window closes
    ...
    /***
     * show
     * Shows the modal window with the signature in the canvas and setups up events for drawing.
     */
    this.show = function()
    {
        this.touchEvent = self.determineEventType();
        ...
        if(self.objsignature.width < canvasWidth)
        {
            canvasWidth = self.objsignature.width;
        }

        if(self.objsignature.height < canvasHeight)
        {
            canvasHeight = self.objsignature.height;
        }
        ...
        // Left Panel
        var leftPanel = $('<div class="imColLeft" style="width: ' + width + 'px; height: ' + (height - 120) + 'px;" />');
        leftPanel.append('<canvas style="margin: 0 auto;display: block;" id="im_Canvas" width="' + canvasWidth + '" height="' + canvasHeight + '"></canvas>');
        ...
        {
            form.append('<a id="btnIMSave" class="button" name="btnIMSave">Save</a>');
            form.append('<a id="btnIMReset" class="button warning" name="btnIMReset">Reset</a>');
            form.append('<a id="btnIMDelete" class="button warning" name="btnIMDelete">Clear</a>');
        }
        ...
        // Draw the signature into the canvas
        self.context.drawImage(self.objsignature, 0, 0);
...
console.log(strDataURI);
            ...
            // Pass a result object with the signature back to the caller
            var objResult = new signatureMarkerResult(strDataURI);
            self.saveMethod(objResult);
            self.close();
        });
...
var signatureMarkerResult = function(signatureData)
{
    this.imageData = signatureData;
};