...
/*		$company = $this->account_model->get_company($this->user->company_id);
	...
	public function retrieve_password()
	{
		...
		$this->email->message('Hello here is your verification code: ' . $code);
	...
	function change_password(){
		...
		$return["password"]  = md5(strtolower($this->input->post('password')) . $salt);
	...
	{	
		// UPDATE site and client statistics
		$sql = "UPDATE sites s SET " .
			"lastinspectiondate = (SELECT MAX(inspection_date) FROM inspections WHERE site_id = s.id AND inspections.deleted = 0), " .
			"num_inspections = (SELECT COUNT(id) FROM inspections WHERE site_id = s.id AND inspections.deleted = 0) " .
			"WHERE s.company_id = " . $this->user->company_id;
			
		$this->db->query($sql);
		
		$sql = "UPDATE clients c SET " .
			"lastinspectiondate = (SELECT MAX(inspection_date) FROM inspections WHERE client_id = c.id AND inspections.deleted = 0), " .
			"num_inspections = (SELECT COUNT(id) FROM inspections WHERE client_id = c.id AND inspections.deleted = 0) " .
			"WHERE c.company_id = " . $this->user->company_id;
		...
		//$tables = array("clients", "sites", "resources", "inspections", "inspectionitems", "inspectionitemphotos", "users", "contacts", "contactsfavourites");
		$tables = array("builders","users");
		...
		foreach($tables as $table)
		{
			...
			{
				$this->db->select("id, company_id, inspectionitem_id, seq_no, photodata_tmb, notes, deleted, created_by");
